...
void mcbcommuniction()
{
    ...
    std::array<uint8_t,48> mcb_ctrl_telegram = GetTelegramData();
    std::cout << sizeof(mcb_ctrl_telegram) <<std::endl;
    for(int index=0;index<sizeof(mcb_ctrl_telegram);index++){
        std::cout <<"index = "<<std::to_string(index)<<" data = ";
        std::cout << std::to_string(mcb_ctrl_telegram[index])<<std::endl;
    }
    while(true){
        // mcb_ctrl_telegram = mcb->mcb_telegram.ctrl.EncodeTelegramArray(&mcb->mcb_telegram.ctrl.fasten,
        //                                                         mcb->mcb_telegram.ctrl.ctrl_telegram_length);
        for(int index=0;index<48;index++){
            comm.SendChar(com_num,mcb_ctrl_telegram[index]);
        }    
