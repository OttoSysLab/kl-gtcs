...
class CrcChecker:
    #region _initial convert array
    # crc32 convert array.
    _crc32_convert_arr = np.array([
    ...
    # Print crcr convert array.
    def print_crc_convert_arr(self):
        """
        dostring
        """
    ...
    def get_crc32_array(self,parr:np.array)->np.array:
        ...
        for idx,val in enumerate(_value):
            _result[idx] = val
            if self._debug_mode==True: print("idx = %d value = %d"%(idx,val))
        
        if self._debug_mode==True:print(_result)
        return _result
