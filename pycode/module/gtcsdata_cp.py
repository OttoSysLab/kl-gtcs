...
class PROCESS(Enum):
    ...
    MCB_S       = "MCB_status"
    # UI handleshake.     
    UI_I        = "UI_in"    # Command form UI to Driver. 
    UI_O        = "UI_out" # Status form Driver to UI.
    # LAN handleshake.
    LAN_I       = "LAN_in"
    LAN_O       = "LAN_out"
    # RS232 handleshake.
...
class GtcsData():
    ...
    __data300_struct = {
        "data_time"        : "datatime",   # DATATIME
        "device_type"      : "int",        # INTEGER
        "device_sn"        : "text",       # TEXT
        "tool_sn"          : "text",       # TEXT
        "selected_job"     : "int",        # INTEGER
        "selected_sequence": "int",        # INTEGER
        "selected_program" : "int",        # INTEGER
        "selected_step"    : "int",        # INTEGER
        "direction"        : "int",        # INTEGER
        "fasten_unit"      : "int",        # INTEGER
        "count_direction"  : "int",        # INTEGER        
        "last_screw_count" : "int",        # INTEGER
        "max_screw_count"  : "int",        # INTEGER
        "fasten_time"      : "real",       # REAL
        "fasten_torque"    : "real",       # REAL
        "fasten_angle"     : "real",       # REAL
        "max_torque"       : "real",       # REAL
        "revolutions"      : "real",       # REAL

        "fasten_status"    : "text",       # TEXT
        "input_io"         : "text",       # TEXT
        "output_io"        : "text",       # TEXT
        "error_masseage"   : "text",       # TEXT
        "tool_count"       : "int",        # INTEGER
        "rpm"              : "int",        # INTEGER
    }
...
class GtcsBulletin():    
    # dictionary
    __dict = {
        PROCESS.M_FSM.value     : None,              # 
        PROCESS.M_Ready.value   : None,              # 
        PROCESS.MCB_C.value     : None,              # 
        PROCESS.MCB_S.value     : None,              # 
        PROCESS.UI_I.value      : None,              # Command form UI to Driver. 
        PROCESS.UI_O.value      : None,              # Status form Driver to UI.
        PROCESS.LAN_I.value     : None,              # 
        PROCESS.LAN_O.value     : None,              # 
        PROCESS.RS232_I.value   : None,              # 
        PROCESS.RS232_O.value   : None,              # 
        PROCESS.RS485_I.value   : None,              # 
        PROCESS.RS485_O.value   : None,              # 
        PROCESS.GPIO_I.value    : None,              # 
        PROCESS.GPIO_O.value    : None,              # 
        PROCESS.DB.value        : None,              # 
        PROCESS.SD.value        : None,              # 
        PROCESS.USB_1.value     : None,              # 
        PROCESS.USB_2.value     : None,              # 
        PROCESS.VIEW.value      : None,              # 
    } 
    
    # 
    __key_list = list(__dict.values())
    for idx,val in enumerate(__key_list):
        _gd = SingleGtcsBulletin()
        __dict.update({val:_gd})
    ...
    # Get bulletin obj.
    def get_bulletin_obj(self,key:str):
        _obj = None
        try:
            _obj = self.__dict.get(key)
        except Exception as e:
            print(e)
        return _obj 

    # Set bulletin obj.
    def set_bulletin_obj(self,key:str,cmd_id:str,data:str)->bool:
        _sgd = SingleGtcsBulletin()
        # Set flag.
        _set_ok = False
        try:
            _sgd.cmd_id = cmd_id
            _sgd.data   = data        
            # update dictionary.
            self.__dict.update({key:_sgd})
            _set_ok = True
        except Exception as e:
            self.__log.error("Set bulletin obj err,msg = %s"%(e))
        return _set_ok
     
    # Get bulletin cmd.
    def get_bulletin_data(self,key:str)->str:
        return 

    # Set bullent cmd.
    def set_bulletin_data(self,key:str)->str:
        return
